...
function getheuristic(matrix, endPos, heuristic,weight) {
    heuristicfunc = null;
    ...
    for (x = 0; x < matrix.length; ++x) {
        ...
        for (y = 0; y < cols; y++) {
            ...
            heur[x][y] = heuristicfunc(endPos, [x, y])*weight;// Math.abs(endPos[0] - x) + Math.abs(endPos[1] - y);//manhattan distance from endpos
            vis[x][y] = 0;
        }
    }
    return heur;
...
function colornode(node, color) {
    if(node[0]===startPos[0] && node[1]===startPos[1]) return;
...
function search(node, g, thr, endPos, allowDiagonal, timelim, matrix, heur, parent, showrec, weight) {
    ...
    for (let i = 0; i < neighl; i++) {
        ...
        if (showrec)
            setTimeout(colornode, 100, node, "rgba(52, 188, 120, 0.15)");
        ...
        if (showrec)
            setTimeout(colornode2, 200, node, "rgba(255, 255, 255, 0.15)");
        path.pop()
   ...
   //clear node, not on recursion stack
...
function idastar(matrix, startPos, endPos, heuristic, allowDiagonal, showrec = true, timelim = 100000, weight) {
    ...
    heur = getheuristic(matrix, endPos, heuristic,weight);
